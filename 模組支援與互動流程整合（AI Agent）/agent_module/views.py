...
class SuggestInputGuidanceView(APIView):
    ...
    def post(self, request):
        ...
        guidance = "您可以輸入想吃的類型、場合、預算等資訊，我們會給您更好的建議！"

        rules = [
            # 類別名稱, 關鍵詞清單, 對應建議文字
            ("飲食偏好", ['不吃辣', '怕辣'], '已排除辣味選項，推薦清爽、湯品等溫和口味'),
            ...
            ("排除語句", ['不想吃', '不要', '不吃'], '已偵測到排除類型，可以幫您濾除不想吃的餐廳'),
        ]

        for _, keywords, response in rules:
            if any(k in user_input for k in keywords):
                guidance = response
                break

        return Response({'guidance': guidance}, status=200)
    ...
    def post(self, request):
        ...
        for r in restaurants:
            ...
            if "拉麵" in name or "拉麵" in address:
                tags.append("拉麵")
            if "牛肉" in name or "牛肉" in address:
                tags.append("牛肉")
            if "咖啡" in name:
                tags.append("咖啡廳")
            if "素" in name:
            ...
            # 地區標籤（自動從地址中擷取區名）
            if "台北市" in address:
                district = address.split("台北市")[-1][:3]
                tags.append(district if district else "台北市")
            elif "新北市" in address:
                district = address.split("新北市")[-1][:3]
                tags.append(district if district else "新北市")
            elif "台中市" in address:
                district = address.split("台中市")[-1][:3]
                tags.append("中市" + district)
            elif "桃園市" in address:
                district = address.split("桃園市")[-1][:3]
                tags.append("市" + district)
            ...
            # --- 距離模擬 ---
            distance = f"{random.randint(200, 1200)} 公尺"
